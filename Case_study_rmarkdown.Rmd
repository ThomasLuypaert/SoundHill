...
### 2.5. Removing the frequencies above 14,400 Hz from the data
...
for (i in 1:length(sample_list)){
  ...
  for(j in 1:length(sample_list[[i]])){
    ...
    sample_list[[i]][[j]]@merged_df <- sample_list[[i]][[j]]@merged_df[47:nrow(sample_list[[i]][[j]]@merged_df),]

    sample_list[[i]][[j]]@binarized_df <- sample_list[[i]][[j]]@binarized_df[47:nrow(sample_list[[i]][[j]]@binarized_df),]
...
for (i in 1:length(inc_mat)){
  ...
  for (j in 1:length(inc_mat[[i]])){

    inc_mat[[i]][[j]] <- unlist(sample_list[[i]][[j]]@binarized_df)

  }


  inc_mat[[i]] <- as.data.frame(t(data.frame(do.call(rbind, inc_mat[[i]]))))
  rownames(inc_mat[[i]]) <-  paste0("OSU", seq(1, nrow(inc_mat[[i]]), 1))
  inc_mat[[i]] <- as.matrix(inc_mat[[i]])

}

names(inc_mat) <- wanted
...
names(inc_mat) <- list_name

inc_mat <- split(inc_mat, names(inc_mat)) %>% map(cbind.data.frame)
...
rarefied_sounddiv <- inc_mat

for (i in 1:length(rarefied_sounddiv)){

  rarefied_sounddiv[[i]] <- NA
...
for (i in 1:length(inc_mat)){
  ...
  rarefied_sounddiv[[i]] <- estimateD(x = inc_mat[[i]],
                                          datatype = "incidence_raw",
                                          base =  "size",
                                          level = 8)

  
  rarefied_sounddiv[[i]] <- distinct(rarefied_sounddiv[[i]][,2:ncol(rarefied_sounddiv[[i]])])
  rarefied_sounddiv[[i]]$site <- names(rarefied_sounddiv)[i]

}

  # Separate the data frames by order of diversity 

rarefied_sounddiv <- rbindlist(rarefied_sounddiv)

for (i in 1:length(rarefied_sounddiv)){

  rarefied_sounddiv_q0 <- subset(rarefied_sounddiv,
                                     rarefied_sounddiv$order==0)


  rarefied_sounddiv_q1 <- subset(rarefied_sounddiv,
                                     rarefied_sounddiv$order==1)


  rarefied_sounddiv_q2 <- subset(rarefied_sounddiv,
                                     rarefied_sounddiv$order==2)
...
rarefied_sounddiv_q0$qD <- (rarefied_sounddiv_q0$qD / (nrow(inc_mat[[1]])))*100
rarefied_sounddiv_q1$qD <- (rarefied_sounddiv_q1$qD / (nrow(inc_mat[[1]])))*100
rarefied_sounddiv_q2$qD <- (rarefied_sounddiv_q2$qD / (nrow(inc_mat[[1]])))*100
...
rarefied_sounddiv_q0$evenness <- rarefied_sounddiv_q2$qD / rarefied_sounddiv_q0$qD

rarefied_sounddiv_q0$evenness.LCL <- rarefied_sounddiv_q2$qD.LCL / rarefied_sounddiv_q0$qD.LCL

rarefied_sounddiv_q0$evenness.UCL <- rarefied_sounddiv_q2$qD.UCL / rarefied_sounddiv_q0$qD.UCL
...
rarefied_sounddiv_q0$site <- toupper(rarefied_sounddiv_q0$site)

rarefied_sounddiv_q0_total <- merge(x=rarefied_sounddiv_q0,
                              y = total_richness,
                              by="site")
...
tax_func_richness_2 <- tax_func_richness +
...
tax_func_evenness <-
  ...
  scale_y_continuous(limits = c(0.52, 0.85))

tax_func_evenness_2 <- tax_func_evenness +
  ggforce::geom_mark_circle(
...
low_richness_aggregated <- soundscapeR::aggregate_df(binarized_soundscape = thresh_list[[29]], output = "incidence_freq")

low_richness_soundscape <- heatmapper(aggregated_soundscape = low_richness_aggregated, type = "regular", annotate = FALSE, maxfreq = 14400) + labs(tag = "A2.") + theme(plot.tag = element_text(size = 24, face = "bold"), axis.text.x = element_text(size=18),
        axis.text.y = element_text(size=18),
        axis.title.x = element_text(size=24),
        axis.title.y = element_text(size=24))+
...
high_richness_aggregated <- soundscapeR::aggregate_df(binarized_soundscape = thresh_list[[41]], output = "incidence_freq")

high_richness_soundscape <- heatmapper(aggregated_soundscape = high_richness_aggregated, type = "regular", annotate = FALSE, maxfreq = 14400) + labs(tag = "A3.") + theme(plot.tag = element_text(size = 24, face = "bold"), axis.text.x = element_text(size=18),
        axis.text.y = element_text(size=18),
        axis.title.x = element_text(size=24),
        axis.title.y = element_text(size=24))+
  scale_x_datetime(breaks = scales::date_breaks("2 hours"), date_labels = "%H:%M", expand = c(0,0)) 

tax_richness <- tax_func_richness_2 + low_richness_soundscape + high_richness_soundscape

# Show what the soundscapes look like for the low/high evenness soundscapes

  # Low evenness soundscape 

low_evenness_aggregated <- soundscapeR::aggregate_df(binarized_soundscape = thresh_list[[4]], output = "incidence_freq")

low_evenness_soundscape <- heatmapper(aggregated_soundscape = low_evenness_aggregated, type = "regular", annotate = FALSE, maxfreq = 14400) + labs(tag = "B2.") + theme(plot.tag = element_text(size = 24, face = "bold"), axis.text.x = element_text(size=18),
        axis.text.y = element_text(size=18),
        axis.title.x = element_text(size=24),
        axis.title.y = element_text(size=24))+
  scale_x_datetime(breaks = scales::date_breaks("2 hours"), date_labels = "%H:%M", expand = c(0,0)) 
...
high_evenness_aggregated <- soundscapeR::aggregate_df(binarized_soundscape = thresh_list[[12]], output = "incidence_freq")

high_evenness_soundscape <- heatmapper(aggregated_soundscape = high_evenness_aggregated, type = "regular", annotate = FALSE, maxfreq = 14400) + labs(tag = "B3.") + theme(plot.tag = element_text(size = 24, face = "bold"), axis.text.x = element_text(size=18),
        axis.text.y = element_text(size=18),
        axis.title.x = element_text(size=24),
        axis.title.y = element_text(size=24))+
  scale_x_datetime(breaks = scales::date_breaks("2 hours"), date_labels = "%H:%M", expand = c(0,0)) 
...
tax_evenness <- tax_func_evenness_2 + low_evenness_soundscape + high_evenness_soundscape

soundscape_total <- tax_richness / tax_evenness
